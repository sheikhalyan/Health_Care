...
> You are currently viewing the **Google Translate** implementation — fast, free-tier friendly, and simple to deploy. For medical-context-aware translation that adapts terminology and tone, see the `master` branch (OpenAI-powered).
...
| **Translation Engine (this branch)** | Google Translate API |
| **Translation Engine (alt branch — `master`)** | OpenAI API — `gpt-3.5-turbo` |
...
To try the OpenAI-powered implementation instead of Google Translate:

```bash
git checkout master
```

The `master` branch swaps the Google Translate API for OpenAI's GPT-3.5-turbo — useful for comparing cost, latency, and translation quality between a traditional translation API and an LLM-based approach.
...
Senior Officer IT at Meezan Bank, Karachi
